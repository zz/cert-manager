...
REGISTRY := registry.git.saybot.net/tools
...
$(DOCKER_PUSH_TARGETS):
	...
	set -e; \
		...
		docker tag $(REGISTRY)/$(APP_NAME)-$(DOCKER_PUSH_CMD):$(BUILD_TAG) $(REGISTRY)/$(APP_NAME)/$(APP_NAME)-$(DOCKER_PUSH_CMD):$${tag} ; \
		docker push $(REGISTRY)/$(APP_NAME)/$(APP_NAME)-$(DOCKER_PUSH_CMD):$${tag}; \
	done
