...
func (s *DomainsService) CreateRecord(domain string, recordAttributes Record) (Record, *Response, string, error) {
	...
	if returnedRecord.Status.Code != "1" {
		return returnedRecord.Record, nil, returnedRecord.Status.Code, fmt.Errorf("!! Could not get domains for create record %s : %s", recordAttributes.Name, returnedRecord.Status.Message)
	}
